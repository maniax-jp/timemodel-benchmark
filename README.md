...
|--------|-----|-----|------|------|-------------------|------|
| **PatchTST (50epoch)** | **5.06** 🥇 | **1.79** 🥇 | **2.25** 🥇 | **15.8%** 🥇 | **0.98ms** ⭐ | 90%学習/10%評価 (3サンプル) |
...
| PatchTST (10epoch) | 11.55 | 2.59 | 3.40 | 21.4% | 0.26ms | 90%学習/10%評価 |
| TimesFM 2.0-500m | 14.94 | 3.18 | 3.86 | 44.5% | 7.3ms | Zero-shot (3サンプル、500Mパラメータ) |

**結論**:
- **🏆 総合優勝: PatchTST (50エポック訓練)**
  - **精度（MSE）**: 5.06 - 全モデル中最高
  - **MAPE**: 15.8% - 全モデル中最も低い相対誤差
  - **推論速度**: 0.98ms/サンプル - 非常に高速
- **Zero-shot最高精度**: TimesFM 1.0-200m（MSE 5.98）
- **モデルサイズと精度の関係**:
  - **Chronos**: tinyからlargeで精度向上（MSE 9.98 → 7.90）✅
  - **TimesFM**: 200mから500mで精度低下（MSE 5.98 → 14.94）⚠️
  - **PatchTST**: 10エポックから50エポックで大幅向上（MSE 11.55 → 5.06）✅✅
- **エポック数の重要性**: PatchTSTは訓練時間を増やすことで最高精度を達成
